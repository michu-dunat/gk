...
def compile_shaders():
    vertex_shader_source = """
        ...
        float rand() {
            return fract(tan(distance(position.xy*PHI, 
            vec2(gl_VertexID, gl_InstanceID))*position.y*PHI)*position.x) * 0.3;
        }
        
        void main(void) {
            gl_Position = P_matrix * V_matrix * M_matrix * 
            (position + vec4(gl_InstanceID % 10 + rand(), gl_InstanceID / 10 + rand(), 0, 0));
