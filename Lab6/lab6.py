...
is_1_clicked, is_2_clicked = False, False
texture1, texture2 = Image.open("musk.tga"), Image.open("dua_lipa.tga")
...
def draw_textured_egg_model_with_triangles():
...
def render(time):
    ...
    # 5.0
    draw_textured_egg_model_with_triangles()
...
def keyboard_key_callback(window, key, scancode, action, mods):
    global is_1_clicked, is_2_clicked

    if key == GLFW_KEY_ESCAPE and action == GLFW_PRESS:
        glfwSetWindowShouldClose(window, GLFW_TRUE)
    if key == GLFW_KEY_1 and action == GLFW_PRESS:
        is_1_clicked = not is_1_clicked
    if key == GLFW_KEY_2 and action == GLFW_PRESS:
        is_2_clicked = not is_2_clicked
